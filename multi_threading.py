...
def alpha_beta_decision(board, turn, ai_level, queue, max_player, current_depth=0):
    ...
    def worker(move):
        ...
        results_queue.put((move, value))

    threads = [Thread(target=worker, args=(move,)) for move in moves]
...
def min_value_parallel(board, turn, player, ai_level, alpha, beta, current_depth, move, results_queue):
...
def max_value_parallel(board, turn, player, ai_level, alpha, beta, current_depth, move, results_queue):
