...
def alpha_beta_decision(board, turn, ai_level, queue, max_player, current_depth=0):
    ...
    for move in board.get_possible_moves():
        ...
        process = Process(target=min_value_parallel, args=(new_board, turn + 1, player % 2 + 1, ai_level, alpha, beta, current_depth + 1, move, player, results_queue))
    ...
    while not results_queue.empty():
        move, value, player_move = results_queue.get()
        if value > best_value:
            best_value = value
            best_move = move

    queue.put((best_move, player_move))
...
def min_value_parallel(board, turn, player, ai_level, alpha, beta, current_depth, move, player_move, results_queue):
    value = min_value(board, turn, player, ai_level, alpha, beta, current_depth)
    results_queue.put((move, value, player_move))
...
def max_value_parallel(board, turn, player, ai_level, alpha, beta, current_depth, move, player_move, results_queue):
    value = max_value(board, turn, player, ai_level, alpha, beta, current_depth)
    results_queue.put((move, value, player_move))
...
class Connect4:
    ...
    def ai_wait_for_move(self):
        if not self.ai_move.empty():
            move, player_move = self.ai_move.get()
            self.move(move)
        else:
